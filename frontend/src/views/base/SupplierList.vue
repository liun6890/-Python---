    ...
    <el-card shadow="never" class="toolbar">
      <el-form :inline="true" @submit.prevent>
        <el-form-item label="关键词">
          <el-input v-model="keyword" placeholder="供应商/联系人/电话" clearable @keyup.enter="fetchList" />
        </el-form-item>
        <el-form-item>
    ...
    <el-dialog v-model="dialogVisible" :title="dialogTitle" width="520px">
      <el-form :model="form" label-width="90px">
        ...
        <el-form-item label="评级">
          <el-input-number v-model="form.rating" :min="0" :max="5" />
        </el-form-item>
...
const saveItem = async () => {
  const payload = { ...form }
  if (payload.id) {
    await request({ url: '/suppliers', method: 'put', data: payload })
    ElMessage.success('更新成功')
  } else {
    await request({ url: '/suppliers', method: 'post', data: payload })
...
const removeItem = async (row) => {
  ...
  await request({ url: '/suppliers', method: 'delete', data: { id: row.id } })
